...
const ExamMarkdown = ({ content }: Props) => {
  ...
  const components = {
    ...
    h2: ({ node, ...props }: any) => <h2 {...props} className="text-xl font-bold mt-10" />,
    ul: ({ children }: any) => (
      <ul className="min-w-[350px] list-disc list-inside my-2">{children}</ul>
    ),
    ol: ({ children }: any) => (
      <ol className="w-min inline-block">{children}</ol>
    ),
    li: ({ children }: any) => (
      <li className="gap-2 leading-relaxed inline-block">
        {children}
      </li>),
